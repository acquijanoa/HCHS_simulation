...
proc printto log = "&homepath./logs/combine_mi_sudaan_pop_&sysdate..log"
			 print= "&homepath./lst/combine_mi_sudaan_pop_&sysdate..lst" new; run; 
*********************************************************************************************************
...
%macro combine_mi_betas(start=1, end=100, corr=exchangeable);
* merge files containing beta estimates using sample data;
data merge_betas;
	set dt_betas.betas_mi_&corr._&start. - dt_betas.betas_mi_&corr._&end.;
...
ods rtf file="&homepath./output/reports/combine_mi_sudaan_&corr._&sysdate..rtf" style=journal bodytitle;
...
%combine_mi_betas(end=91, corr=exchangeable);
*%combine_mi_betas(end=100, corr=independent);
